...
// Map CRCs to their strings.
MapCRCName fx_crc, node_crc;
MapCRCName mat_crc;
MapCRCName mesh_crc;
MapCRCName msg_hash;
...
// Name table for Alchemy CRCs.
static const LPCSTR fx_names[] =
{
	"AirField_Approach",
	"AirField_Magnitude",
	"Appearance_LODCurve",
...
void init_fx_crc()
{
	// Create the CRCs for the Alchemy names.
	for (int i = 0; fx_names[i]; ++i)
	{
		fx_crc[fl_crc32_b((BYTE *)fx_names[i], strlen(fx_names[i]))] = fx_names[i];
	}

	// This one doesn't match.
	fx_crc[0x1C65B7B9] = "BeamApp_LineAppearance";
}


// Given a CRC, return either its name or hex value.
LPCSTR crc_name(const MapCRCName& crcmap, DWORD crc)
{
	static char buf[16];

	CIterCRCName iter = crcmap.find(crc);
	if (iter == crcmap.end())
	{
		sprintf(buf, "0x%08X", crc);
		return buf;
	}
	return iter->second;
}
